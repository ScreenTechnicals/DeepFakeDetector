...
```bash
# Register
curl -X POST http://localhost:8001/register -d "username=user&password=pass"

# Login (returns JWT)
curl -X POST http://localhost:8001/token -d "username=user&password=pass"

# Use token for protected endpoints (/history, /users/me)
curl -H "Authorization: Bearer <token>" http://localhost:8001/history
```
