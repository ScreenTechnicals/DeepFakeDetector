...
export const exportResults = (result, fileName) => {
  ...
  const exportFileDefaultName = `deepforensics_analysis_${fileName}_${new Date().toISOString().slice(0,10)}.json`;
...
export const downloadPDF = (result, fileName, preview) => {
  ...
  pdf.text('DeepForensics Analysis Report', 20, 20);
  ...
  pdf.text('Generated by DeepForensics - Advanced AI Deepfake Detection', 20, 280);
  
  // Save the PDF
  pdf.save(`deepforensics_report_${fileName}_${new Date().toISOString().slice(0,10)}.pdf`);
};